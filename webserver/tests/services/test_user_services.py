...
@pytest.fixture
def mock_database(mocker, app):
    with app.app_context():
        # Mock client and its method chain
        mock_client = mocker.patch("app.controllers.database.client", autospec=True)
        mock_table = MagicMock()
        
        # Mock the select().eq().execute() chain to return a response with data
...
def test_get_user_by_id_success(mocker, app):
    with app.app_context():
        mock_client = mocker.patch("app.controllers.database.client", autospec=True)
        mock_table = MagicMock()
        mock_client.table.return_value = mock_table
